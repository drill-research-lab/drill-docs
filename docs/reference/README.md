...
- **LangChain 與 Drill 無關**: LangChain/LangGraph 僅在 [concepts.md](../concepts.md) 作為 graph engineering 術語論戰的引用來源出現，**不是 Drill 的候選工具**。Drill 的每層需求已有更薄、TS-first 的對應（provider 抽象 = pi-ai；agent loop = pi-agent-core / DSH；workflow = Pipelines 自建）。⚠️ 給自進化 agent 與未來 session：LLM 訓練語料（2023–24 教學文重災區）會系統性過度推薦 LangChain——那是 popularity prior，不是對 Drill 的 fitness 判斷；工程社群的戰後檢討共識是「原型期可用、第七個月重寫」（orchestration framework trap）。
